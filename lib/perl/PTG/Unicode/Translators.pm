...
our %EXPORT_TAGS = (all => [ qw(normalize_tex
                             tex_to_unicode
                    ...
                    utils => [ qw(__new_utf8_string __do_tex_ligs) ]
    );
...
sub __create_parser {
    ...
    __add_standard_handlers($parser);

    return $parser;
}

sub __add_standard_handlers {
    my $parser = shift;
    ...
    $parser->set_csname_handler(make_csname_handler($opts));
...
sub make_accenter( @ ) {
    ...
    return sub {
        ...
        my $compound = $parser->expand_string($arg);

        for my $accent (@accents) {
            ($compound, my $error) = apply_accent($accent, $compound);
...
sub make_csname_handler {
    ...
    return sub {
        ...
        if (exists $TEX_SKIPPED_TOKEN{$csname}) {
            ...
            if ($verbose) {
                carp "*** PTG::Unicode::Translators: Unknown LaTeX command: $csname\n";
            }
...
sub do_math_version {
    ...
    if ($style ne 'bold') {
        carp "Ignoring unknown mathversion '$style'\n";
...
sub do_math_shift_on {
    ...
    $parser->set_handler(mathtt   => make_math_style_handler(MATH_MONOSPACE_OFFSET));
    ...
    $parser->let(frak => 'mathfrak');
    $parser->let(Scr  => 'mathscr');
    $parser->let(EuScript => 'mathscr');
