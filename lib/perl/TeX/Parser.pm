...
my %expand_defs_of :BOOLEAN(:name<expand_defs> :get<expand_defs> :default<0>);
...
sub null_handler($$) {
    ...
    if ($parser->buffer_output()) {
        $output_buffer_of{ident $parser} .= $token
    }
...
sub par {
    ...
    my $par_handler = $parser->get_handler('par');

    if (defined $par_handler) {
        $par_handler->($parser, PAR_TOKEN);
    }
...
sub read_alphabetic_constant {
    ...
    if ($token == CATCODE_CSNAME) {
        ...
        if (length($char) != 1) {
            ...
            $LOG->fatal("Improper alphabetic constant \\$char on line $line");
        }
...
sub read_literal_integer {
    ...
    $LOG->fatal("Invalid integer: '$next_token' on line $line\n");
}
    ...
    while (my $token = $self->get_maybe_expanded_token($expanded)) {
        ...
        if ($token == CATCODE_PARAMETER) {
            if (! $def) {
                ...
                $LOG->fatal("Not a definition: You can't use the macro parameter $token here on line $line\n");
            }
            ...
            if ($next == CATCODE_PARAMETER) {
                ...
                $LOG->fatal("You can't the macro parameter $token before $next on line $line\n");
            }
...
sub read_parameter_text {
    ...
    while (my $token = $self->get_next_token()) {
        ...
        if ($token == CATCODE_END_GROUP) {
            $LOG->fatal("Illegal end of group $token in parameter text ",
                        "on line ",
                        $self->get_line_no(),
                        " of ",
                        $self->get_file_name(),
            ...
            if ($next_token == CATCODE_OTHER) {
                ...
                if ($char =~ /^[0-9]$/) {
                    if ($char == ++$max_arg) {
                        ...
                        $LOG->fatal("Parameter numbers must be consecutive ",
                                    "on line ",
                                    $self->get_line_no(),
                                    " of ",
                                    $self->get_file_name(),
...
sub read_macro_parameters {
    ...
    if (@_ == 1) {
        if (UNIVERSAL::isa($_[0], 'TeX::TokenList') || UNIVERSAL::isa($_[0], 'ARRAY')) {
            @parameter_text = @{ $_[0] };
        }
    } else {
        @parameter_text = @_;
...
sub define_macro {
    ...
    my $macro = make_macro($param_text, $macro_text);

    $tex->set_handler($csname, sub { $macro->expand(@_) });
...
sub do_def {
    ...
    if ($control_sequence->length() != 1) {
        ...
        carp("Malformed ${token}{$control_sequence} on line $line_no\n");
...
sub expand_string {
    ...
    $sub_parser->set_default_handler(\$expansion);
