...
my %SIMPLE_FIELD = array_to_hash qw(address arxiv book booktitle conference
...
sub retrieve_xreffed_bibitem {
    ...
    if (eval { $xref_key->isa("TeX::AMSrefs::BibItem") }) {
        ...
        if (! defined $container) {
            $LOG->error("No AMSrefs container to resolve xref '$xref_key' ",
                        "in ", $self->get_citekey(), "\n");
        ...
        if (defined $xref) {
            ...
            $LOG->error("Xref '$xref_key' undefined\n");
        }
    }
}
    ...
    if (my $xref = $self->find_xref()) {
        ...
        while (my ($key, $entry) = each %xref_entries) {
            if ($SIMPLE_FIELD{$key}) {
                ...
                $LOG->error("Unknown key in xref: '$key'\n");
            }
        }
    }
    ...
    if (eval { $field->isa('TeX::AMSrefs::BibItem') }) {
        ...
        if (defined $container) {
            ...
            if (defined $inner_item) {
                ...
                $LOG->error("Xref '$field' undefined\n");
                return;
            }
        } else {
            $LOG->error("No AMSrefs container to resolve xref '$field'",
                        " in ", $self->get_citekey(), "\n");
        }
    }
}
        ...
        if (! ref($value) && $value eq lc($value)) {
            ...
            if (defined $container) {
                ...
                if (defined $xref) {
                    ...
                    $LOG->error("Abbreviation '$value' undefined in $new_key in $cite_key\n");
                }
            } else {
                $LOG->error("No AMSrefs container to resolve abbreviation '$value'",
                            " in ", $self->get_citekey(), "\n");
            }
        }
    }
